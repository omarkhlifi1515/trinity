...
from trinity.hr.doctype.expense_claim.expense_claim import (	MismatchError,
	get_outstanding_amount_for_claim,
	make_bank_entry,
	make_expense_claim_for_delivery_trip,
)
from trinity.tests.utils import HRMSTestSuite
...
class TestExpenseClaim(HRMSTestSuite):
	...
	def test_expense_claim_against_fully_paid_advances(self):
		from trinity.hr.doctype.employee_advance.test_employee_advance import (			get_advances_for_claim,
	...
	def test_expense_claim_with_deducted_returned_advance(self):
		from trinity.hr.doctype.employee_advance.test_employee_advance import (			create_return_through_additional_salary,
			get_advances_for_claim,
			make_employee_advance,
			make_journal_entry_for_advance,
		)
		from trinity.hr.doctype.expense_claim.expense_claim import get_allocation_amount		from trinity.payroll.doctype.salary_component.test_salary_component import create_salary_component		from trinity.payroll.doctype.salary_structure.test_salary_structure import make_salary_structure
	...
	def test_multicurrency_claim(self):
		from trinity.hr.doctype.employee_advance.test_employee_advance import (			create_advance_account,
			get_advances_for_claim,
			make_employee_advance,
			make_payment_entry,
		)
	...
	def test_expense_claim_status_as_payment_after_unreconciliation(self):
		from trinity.hr.doctype.employee_advance.test_employee_advance import make_payment_entry
		payable_account = get_payable_account(company_name)
