...
def subscription_updated(app: str, plan: str):
	if app in ["trinity", "erpnext"] and plan:
		update_erpnext_access()


def update_erpnext_access(user_input: dict | None):
	"""
	Called from hooks after setup wizard completion, ignored if user has no trinity subscription
	enables erpnext workspaces and roles if user has subscribed to both trinity and erpnext
	disables erpnext workspaces and roles if user has subscribed to trinity but not erpnext
	"""
...
def set_app_logo():
	frappe.db.set_single_value("Navbar Settings", "app_logo", "/assets/trinity/images/trinity-logo.svg")


def get_erpnext_roles() -> set:
	erpnext_roles = get_roles_for_app("erpnext")
	hrms_roles = get_roles_for_app("trinity")
